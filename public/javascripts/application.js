...
MapCanvas = {
  ...
  bindEvents: function(currentCity,year) {
    $('#chart_div').html("");
    MapCanvas.constructPie(currentCity.piechart[year],year);
    MapCanvas.constructBar(currentCity.barchart[year],year);
    MapCanvas.constructInfo(currentCity.table[year],year);
    $("#content").show();
  },
  init: function(map) {
    this.opt = {
  ...
  selectCity: function(){
    $.getJSON("/constituencies/" + jQuery("#search_box").val(), function(json) {
      MapCanvas.handleJson(json);
    });
  },
  changeCity:function(){
    $.getJSON("/constituencies/" + $("#search_box").val(), function(json) {
      MapCanvas.handleJson(json);
    });
  }
};
...
function onLoadFunction() {
  ...
  GEvent.addListener(GoogleMap.map, "click", function(overlay, latlng) {
    GoogleMap.selectCity(latlng);
    return false;
  });
  $("#search_box").change(function(e) {
    MapCanvas.changeCity($(this).val());
    return false;
